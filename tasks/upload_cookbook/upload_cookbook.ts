...
function installChefDK() {
  ...
  if (!fs.existsSync("/opt/chefdk")) {
    ...
    try {
      ...
      let curl_exit_code: number = tl.tool("curl").line("https://omnitruck.chef.io/install.sh --output /tmp/chefdk_install.sh").exec();
      let install_exit_code: number = tl.tool("bash").line("/tmp/chefdk_install.sh -s -- -c current -P chefdk").exec();
    } catch (err) {

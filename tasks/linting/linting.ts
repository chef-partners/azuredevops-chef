...
async function run() {
    ...
    if (chef_installed) {
        ...
        // output information about what is being run
        console.log("Linting Action: %s", params["lint"]["action"]);

        // build up the command that is to be run
        let command = builtin_settings["paths"]["chef"];
        let command_arguments = sprintf("exec rake %s", params["lint"]["actionCommand"]);

        try {
            tl.debug(sprintf("Lint command: %s %s", command, command_arguments));
            let command_result: number = await tl.tool(command)
                                .line(command_arguments)
                                .exec(<any>{cwd: path.normalize(params["lint"]["folder"])});
        } catch (err) {
